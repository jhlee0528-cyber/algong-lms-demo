...
import mathCreateMission from "./mathCreateMission.vue";
import MissionResult from "../MissionResult.vue";
import MathMissionEdit from "./mathMissionEdit.vue";
    ...
    const fetchMissions = async () => {
      try {
        ...
        if (response.data && response.data.data) {
          const mathMissions = response.data.data
            .filter((mission) => mission.subject === "math")
            .sort((a, b) => new Date(a.createdDate) - new Date(b.createdDate));

          missions.value = mathMissions;
        }
      } catch (error) {
